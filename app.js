...
const data = [
    {
        title: 'Соглашение о конфиденциальности б/н',
        regnum: 'Вх-1321/18',
        regdate: '07.09.2018',
        author: 'Журбинский Владимир'
    },
    {
        title: 'Акт сверки за 1 полугодие 2018',
        regnum: 'Вх-1318/18',
        regdate: '07.09.2018',
        author: 'Журбинский Владимир'
    },
    {
        title: 'О продлении действия льготного периода №231 от 30.08.2018',
        regnum: 'Вх-1309/18',
        regdate: '06.09.2018',
        author: 'Стриго Олег'
    },
    {
        title: 'Счета за август+ протокол согласования цены №4 от 01.08.2018',
        regnum: 'Вх-1285/18',
        regdate: '04.09.2018',
        author: 'Стриго Олег'
    },
    {
        title: 'Сертификаты + сопроводительное письмо №8310 от 16.08.2018',
        regnum: 'Вх-1211/18',
        regdate: '29.08.2018',
        author: 'Юферев Андрей'
    }
];
...
bot.command('start', ({ from: { username, first_name, last_name }, reply }) => {
    return reply(
        `Добро пожаловать, ${first_name} ${last_name}! Ваш ID: @${username}`,
        Markup.keyboard([
...
bot.hears(/вопрос/i, ctx =>
    ctx.replyWithMarkdown('Опишите Ваш вопрос, максимально подробно.')
);
bot.hears(/техподдержка|поломка/i, ctx => {
    ctx.replyWithMarkdown('*Сейчас по шагам опишите ситуацию.*').then(() => {
        ...
        bot.on('text', ctx => {
            ...
            json = ctx.update;
            ctx.replyWithMarkdown(
                '*Спасибо, текст заявки сохранен!\nТП. Шаг 2 - критичность.*',
        ...
        bot.on(['sticker', 'photo'], ctx => {
            ...
            bot.telegram.getFileLink(photoId).then(result => {
                ...
                request
                    ...
                    .pipe(
                        fse.createWriteStream(path.join(filesPath, filename))
                    );
                fse.writeFile(
                    file,
                    ...
                    function(err) {
                        if (err) {
                            console.error(err);
                            ctx.reply(
                                'Ошибка синхронизации с СЭД! Попробуйте еще раз.'
                            );
                            return;
                        }
                        console.log(
                ...
                ctx.replyWithMarkdown(
                    '*Спасибо, заявка отправлена в СЭД для модерации!*'
                );
            });
        });
    });
...
bot.hears(/отгул/i, ctx =>
    ctx.replyWithMarkdown(
        'Пожалуйста опишите ситуацию в формате:\n_[причина отгула]_, с _[дата начала в формате ДД.ММ.ГГ]_ до _[дата окончания в формате ДД.ММ.ГГ]_.\nПри выборе даты вы можете использовать команду [/calendar]'
    )
);
bot.hears(/о чат-боте/i, ctx =>
    ctx.replyWithMarkdown(
        'Я - бот-помощник для работы с СЭД на базе EOSfSP.\nПо всем вопросам обращаться:\nsupport@junicsoft.ru'
    )
);
bot.hears(/поиск/i, ctx => {
    ctx.replyWithMarkdown('*Активные документы:*').then(() => {
        data.forEach(item => {
            ctx.replyWithMarkdown(
                `[${item.regnum}](https://junicsoft.ru) от ${item.regdate}`
            );
        });
    });
});
...
bot.telegram.getMe().then(bot_informations => {
    ...
    console.log(
        'Server has initialized bot nickname. Nick: ' +
